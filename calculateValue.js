...
function calculateValue(
  ...
  const amountOutMin = minAmountOut.add(
    slippage.mul(BigNumber.from(5)).div(BigNumber.from(100))
  );

  const buyAmount = BigNumber.from(
    sqrt(
      BigNumber.from(3964107892000)
        .mul(reserveWeth)
        .mul(reserveToken)
        .mul(amountIn)
        .mul(amountOutMin)
        .add(
          BigNumber.from(988053892081)
            .mul(pow(amountIn, 2))
            .mul(pow(amountOutMin, 2))
        )
    )
